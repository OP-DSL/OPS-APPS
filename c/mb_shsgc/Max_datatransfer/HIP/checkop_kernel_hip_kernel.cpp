...
int size0 ){
  ...
  if (size0==-1) dims_checkop_kernel[0][0]=0;
  ...
  for (int d=0; d<1; d++)
    ops_reduction_hip<OPS_INC>(&arg3[d+(hipBlockIdx_x + hipBlockIdx_y*hipBlockDim_x)*1],arg3_l[d]);
  for (int d=0; d<1; d++)
    ops_reduction_hip<OPS_INC>(&arg4[d+(hipBlockIdx_x + hipBlockIdx_y*hipBlockDim_x)*1],arg4_l[d]);
  for (int d=0; d<1; d++)
    ops_reduction_hip<OPS_INC>(&arg5[d+(hipBlockIdx_x + hipBlockIdx_y*hipBlockDim_x)*1],arg5_l[d]);
...
void ops_par_loop_checkop_kernel_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_checkop_kernel_h[0][0] || xdim1 != dims_checkop_kernel_h[1][0] || xdim2 != dims_checkop_kernel_h[2][0]) {
    ...
    hipSafeCall(block->instance->ostream(), hipMemcpyToSymbol(HIP_SYMBOL(dims_checkop_kernel), dims_checkop_kernel_h, sizeof(dims_checkop_kernel)));
  }
  ...
  if (x_size > 0)
    hipLaunchKernelGGL(ops_checkop_kernel,grid ,tblock ,nshared ,0 , (double *)p_a[0], (double *)p_a[1],
