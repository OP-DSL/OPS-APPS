...
int size0 ){
  ...
  if (size0==-1) dims_save_kernel[0][0]=0;
...
void ops_par_loop_save_kernel_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_save_kernel_h[0][0] || xdim1 != dims_save_kernel_h[1][0] || xdim2 != dims_save_kernel_h[2][0] || xdim3 != dims_save_kernel_h[3][0] || xdim4 != dims_save_kernel_h[4][0] || xdim5 != dims_save_kernel_h[5][0]) {
    ...
    hipSafeCall(block->instance->ostream(), hipMemcpyToSymbol(HIP_SYMBOL(dims_save_kernel), dims_save_kernel_h, sizeof(dims_save_kernel)));
  }
  ...
  if (x_size > 0)
    hipLaunchKernelGGL(ops_save_kernel,grid ,tblock ,0 ,0 , (double *)p_a[0], (double *)p_a[1],
