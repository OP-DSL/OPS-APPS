...
int size0 ){
  ...
  if (size0==-1) dims_gridgen_kernel[0][0]=0;
...
void ops_par_loop_gridgen_kernel_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_gridgen_kernel_h[0][0]) {
    ...
    hipSafeCall(block->instance->ostream(), hipMemcpyToSymbol(HIP_SYMBOL(dims_gridgen_kernel), dims_gridgen_kernel_h, sizeof(dims_gridgen_kernel)));
  }
  ...
  if (x_size > 0)
    hipLaunchKernelGGL(ops_gridgen_kernel,grid ,tblock ,0 ,0 , (double *)p_a[0], arg_idx[0],x_size);
