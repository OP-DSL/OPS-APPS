    ...
    void buildOpenCLKernels_vars_kernel(OPS_instance *instance, int xdim0, int xdim1, int xdim2, int xdim3, int xdim4) {
      ...
      if(!isbuilt_vars_kernel) {
        ...
        instance->ostream() <<"Compiling vars_kernel "<<OCL_FMA<<" source -- start \n";
          ...
          if (pPath!=NULL)
            if(OCL_FMA)
              sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_vars_kernel=%d  -Dxdim1_vars_kernel=%d  -Dxdim2_vars_kernel=%d  -Dxdim3_vars_kernel=%d  -Dxdim4_vars_kernel=%d ", pPath, 32,xdim0,xdim1,xdim2,xdim3,xdim4);
            else
              sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_vars_kernel=%d  -Dxdim1_vars_kernel=%d  -Dxdim2_vars_kernel=%d  -Dxdim3_vars_kernel=%d  -Dxdim4_vars_kernel=%d ", pPath, 32,xdim0,xdim1,xdim2,xdim3,xdim4);
          else {
