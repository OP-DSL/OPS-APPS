...
//
    ...
    static bool isbuilt_calvar_kernel = false;

    void buildOpenCLKernels_calvar_kernel(OPS_instance *instance, int xdim0, int xdim1, int xdim2, int xdim3, int xdim4) {

      //int ocl_fma = OCL_FMA;
      if(!isbuilt_calvar_kernel) {
        ...
        instance->ostream() <<"Compiling calvar_kernel "<<OCL_FMA<<" source -- start \n";
          ...
          if (pPath!=NULL)
            if(OCL_FMA)
              sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_calvar_kernel=%d  -Dxdim1_calvar_kernel=%d  -Dxdim2_calvar_kernel=%d  -Dxdim3_calvar_kernel=%d  -Dxdim4_calvar_kernel=%d ", pPath, 32,xdim0,xdim1,xdim2,xdim3,xdim4);
            else
              sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_calvar_kernel=%d  -Dxdim1_calvar_kernel=%d  -Dxdim2_calvar_kernel=%d  -Dxdim3_calvar_kernel=%d  -Dxdim4_calvar_kernel=%d ", pPath, 32,xdim0,xdim1,xdim2,xdim3,xdim4);
          else {
          ...
          instance->ostream() << "compiling calvar_kernel -- done\n";

        // Create the OpenCL kernel
        instance->opencl_instance->OPS_opencl_core.kernel[3] = clCreateKernel(instance->opencl_instance->OPS_opencl_core.program, "ops_calvar_kernel", &ret);
        clSafeCall( ret );

        isbuilt_calvar_kernel = true;
        free(source_str[0]);
      }
...
void ops_par_loop_calvar_kernel(char const *name, ops_block block, int dim, int* range,
 ...
 ops_arg arg4) {
  ...
  if (block->instance->OPS_diags > 1) {
    ops_timing_realloc(block->instance,3,"calvar_kernel");
  ...
  buildOpenCLKernels_calvar_kernel(block->instance,
  ...
  if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[3], 5, sizeof(cl_double), (void*) &gam1 ));
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[3], 8, sizeof(cl_int), (void*) &base2 ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[3], 9, sizeof(cl_int), (void*) &base3 ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[3], 10, sizeof(cl_int), (void*) &base4 ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[3], 11, sizeof(cl_int), (void*) &x_size ));
