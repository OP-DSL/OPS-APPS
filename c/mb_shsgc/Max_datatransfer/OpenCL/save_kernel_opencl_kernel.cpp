...
//
    ...
    static bool isbuilt_save_kernel = false;

    void buildOpenCLKernels_save_kernel(OPS_instance *instance, int xdim0, int xdim1, int xdim2, int xdim3, int xdim4, int xdim5) {

      //int ocl_fma = OCL_FMA;
      if(!isbuilt_save_kernel) {
        ...
        instance->ostream() <<"Compiling save_kernel "<<OCL_FMA<<" source -- start \n";
          ...
          if (pPath!=NULL)
            if(OCL_FMA)
              sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_save_kernel=%d  -Dxdim1_save_kernel=%d  -Dxdim2_save_kernel=%d  -Dxdim3_save_kernel=%d  -Dxdim4_save_kernel=%d  -Dxdim5_save_kernel=%d ", pPath, 32,xdim0,xdim1,xdim2,xdim3,xdim4,xdim5);
            else
              sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_save_kernel=%d  -Dxdim1_save_kernel=%d  -Dxdim2_save_kernel=%d  -Dxdim3_save_kernel=%d  -Dxdim4_save_kernel=%d  -Dxdim5_save_kernel=%d ", pPath, 32,xdim0,xdim1,xdim2,xdim3,xdim4,xdim5);
          else {
          ...
          instance->ostream() << "compiling save_kernel -- done\n";

        // Create the OpenCL kernel
        instance->opencl_instance->OPS_opencl_core.kernel[2] = clCreateKernel(instance->opencl_instance->OPS_opencl_core.program, "ops_save_kernel", &ret);
        clSafeCall( ret );

        isbuilt_save_kernel = true;
        free(source_str[0]);
      }
...
void ops_par_loop_save_kernel(char const *name, ops_block block, int dim, int* range,
 ...
 ops_arg arg4, ops_arg arg5) {
  ...
  buildOpenCLKernels_save_kernel(block->instance,
  ...
  if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[2], 8, sizeof(cl_int), (void*) &base2 ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[2], 9, sizeof(cl_int), (void*) &base3 ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[2], 10, sizeof(cl_int), (void*) &base4 ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[2], 11, sizeof(cl_int), (void*) &base5 ));
