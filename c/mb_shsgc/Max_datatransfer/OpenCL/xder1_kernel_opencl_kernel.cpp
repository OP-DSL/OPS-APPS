...
//
    ...
    static bool isbuilt_xder1_kernel = false;

    void buildOpenCLKernels_xder1_kernel(OPS_instance *instance, int xdim0, int xdim1) {

      //int ocl_fma = OCL_FMA;
      if(!isbuilt_xder1_kernel) {
        ...
        instance->ostream() <<"Compiling xder1_kernel "<<OCL_FMA<<" source -- start \n";
          ...
          if (pPath!=NULL)
            if(OCL_FMA)
              sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_xder1_kernel=%d  -Dxdim1_xder1_kernel=%d ", pPath, 32,xdim0,xdim1);
            else
              sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_xder1_kernel=%d  -Dxdim1_xder1_kernel=%d ", pPath, 32,xdim0,xdim1);
          else {
          ...
          instance->ostream() << "compiling xder1_kernel -- done\n";

        // Create the OpenCL kernel
        instance->opencl_instance->OPS_opencl_core.kernel[4] = clCreateKernel(instance->opencl_instance->OPS_opencl_core.program, "ops_xder1_kernel", &ret);
        clSafeCall( ret );

        isbuilt_xder1_kernel = true;
        free(source_str[0]);
      }
...
void ops_par_loop_xder1_kernel(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  buildOpenCLKernels_xder1_kernel(block->instance,
  xdim0,xdim1);
  ...
  if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[4], 2, sizeof(cl_double), (void*) &dx ));
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[4], 5, sizeof(cl_int), (void*) &x_size ));
