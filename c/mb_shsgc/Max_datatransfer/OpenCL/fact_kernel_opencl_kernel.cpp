...
//
    ...
    static bool isbuilt_fact_kernel = false;

    void buildOpenCLKernels_fact_kernel(OPS_instance *instance, int xdim0, int xdim1) {
      ...
      if(!isbuilt_fact_kernel) {
        ...
        instance->ostream() <<"Compiling fact_kernel "<<OCL_FMA<<" source -- start \n";
          ...
          if (pPath!=NULL)
            if(OCL_FMA)
              sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_fact_kernel=%d  -Dxdim1_fact_kernel=%d ", pPath, 32,xdim0,xdim1);
            else
              sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_fact_kernel=%d  -Dxdim1_fact_kernel=%d ", pPath, 32,xdim0,xdim1);
          else {
          ...
          instance->ostream() << "compiling fact_kernel -- done\n";

        // Create the OpenCL kernel
        instance->opencl_instance->OPS_opencl_core.kernel[12] = clCreateKernel(instance->opencl_instance->OPS_opencl_core.program, "ops_fact_kernel", &ret);
        clSafeCall( ret );

        isbuilt_fact_kernel = true;
        free(source_str[0]);
      }
...
void ops_par_loop_fact_kernel(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  buildOpenCLKernels_fact_kernel(block->instance,
  xdim0,xdim1);
  ...
  if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[12], 2, sizeof(cl_double), (void*) &dx ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[12], 3, sizeof(cl_double), (void*) &dt ));
