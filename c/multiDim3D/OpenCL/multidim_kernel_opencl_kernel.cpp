...
//
    ...
    void buildOpenCLKernels_multidim_kernel(OPS_instance *instance, int xdim0, int ydim0, int zdim0) {
      ...
      if(!isbuilt_multidim_kernel) {
        ...
        instance->ostream() <<"Compiling multidim_kernel "<<OCL_FMA<<" source -- start \n";
          ...
          if (pPath!=NULL)
            if(OCL_FMA)
              sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_multidim_kernel=%d  -Dydim0_multidim_kernel=%d  -Dzdim0_multidim_kernel=%d ", pPath, 32,xdim0,ydim0,zdim0);
            else
              sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_multidim_kernel=%d  -Dydim0_multidim_kernel=%d  -Dzdim0_multidim_kernel=%d ", pPath, 32,xdim0,ydim0,zdim0);
          else {
...
void ops_par_loop_multidim_kernel(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[0], 4, sizeof(cl_int), (void*) &arg_idx[2] ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[0], 5, sizeof(cl_int), (void*) &x_size ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[0], 6, sizeof(cl_int), (void*) &y_size ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[0], 7, sizeof(cl_int), (void*) &z_size ));
