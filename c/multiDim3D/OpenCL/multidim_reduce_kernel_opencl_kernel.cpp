...
//
    ...
    void buildOpenCLKernels_multidim_reduce_kernel(OPS_instance *instance, int xdim0, int ydim0, int zdim0) {
      ...
      if(!isbuilt_multidim_reduce_kernel) {
        ...
        instance->ostream() <<"Compiling multidim_reduce_kernel "<<OCL_FMA<<" source -- start \n";
          ...
          if (pPath!=NULL)
            if(OCL_FMA)
              sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_multidim_reduce_kernel=%d  -Dydim0_multidim_reduce_kernel=%d  -Dzdim0_multidim_reduce_kernel=%d ", pPath, 32,xdim0,ydim0,zdim0);
            else
              sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_multidim_reduce_kernel=%d  -Dydim0_multidim_reduce_kernel=%d  -Dzdim0_multidim_reduce_kernel=%d ", pPath, 32,xdim0,ydim0,zdim0);
          else {
...
void ops_par_loop_multidim_reduce_kernel(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[2], 7, sizeof(cl_int), (void*) &z_size ));
