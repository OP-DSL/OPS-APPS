...
//
    ...
    void buildOpenCLKernels_multidim_copy_kernel(OPS_instance *instance, int xdim0, int ydim0, int zdim0, int xdim1, int ydim1, int zdim1) {
      ...
      if(!isbuilt_multidim_copy_kernel) {
        ...
        instance->ostream() <<"Compiling multidim_copy_kernel "<<OCL_FMA<<" source -- start \n";
          ...
          if (pPath!=NULL)
            if(OCL_FMA)
              sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_multidim_copy_kernel=%d  -Dydim0_multidim_copy_kernel=%d  -Dzdim0_multidim_copy_kernel=%d  -Dxdim1_multidim_copy_kernel=%d  -Dydim1_multidim_copy_kernel=%d  -Dzdim1_multidim_copy_kernel=%d ", pPath, 32,xdim0,ydim0,zdim0,xdim1,ydim1,zdim1);
            else
              sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_multidim_copy_kernel=%d  -Dydim0_multidim_copy_kernel=%d  -Dzdim0_multidim_copy_kernel=%d  -Dxdim1_multidim_copy_kernel=%d  -Dydim1_multidim_copy_kernel=%d  -Dzdim1_multidim_copy_kernel=%d ", pPath, 32,xdim0,ydim0,zdim0,xdim1,ydim1,zdim1);
          else {
          ...
          instance->ostream() << "compiling multidim_copy_kernel -- done\n";

        // Create the OpenCL kernel
        instance->opencl_instance->OPS_opencl_core.kernel[1] = clCreateKernel(instance->opencl_instance->OPS_opencl_core.program, "ops_multidim_copy_kernel", &ret);
        clSafeCall( ret );

        isbuilt_multidim_copy_kernel = true;
        free(source_str[0]);
      }
...
void ops_par_loop_multidim_copy_kernel(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  buildOpenCLKernels_multidim_copy_kernel(block->instance,
  xdim0,ydim0,zdim0,xdim1,ydim1,zdim1);
  ...
  if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[1], 6, sizeof(cl_int), (void*) &z_size ));
