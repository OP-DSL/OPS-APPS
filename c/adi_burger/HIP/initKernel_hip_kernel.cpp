...
//
__constant__ int dims_initKernel [3][1];
static int dims_initKernel_h [3][1] = {{0}};

//user function
__device__

void initKernel_gpu(ACC<double> &uVel,
  ACC<double> &vVel,
  int *idx) {
    uVel(0, 0) =
        0.75 -
...
int size1 ){
  ...
  arg0 += idx_x * 1*1 + idx_y * 1*1 * dims_initKernel[0][0];
  arg1 += idx_x * 1*1 + idx_y * 1*1 * dims_initKernel[1][0];

  if (idx_x < size0 && idx_y < size1) {
    ACC<double> argp0(dims_initKernel[0][0], arg0);
    ACC<double> argp1(dims_initKernel[1][0], arg1);
    initKernel_gpu(argp0, argp1, arg_idx);
  }
...
void ops_par_loop_initKernel_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_initKernel_h[0][0] || xdim1 != dims_initKernel_h[1][0]) {
