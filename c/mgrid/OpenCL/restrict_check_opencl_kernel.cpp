...
//
    ...
    static bool isbuilt_restrict_check = false;

    void buildOpenCLKernels_restrict_check(OPS_instance *instance, int xdim0) {

      //int ocl_fma = OCL_FMA;
      if(!isbuilt_restrict_check) {
        buildOpenCLKernels(instance);
        //clSafeCall( clUnloadCompiler() );
        cl_int ret;
        char* source_filename[1] = {(char*)"./OpenCL/restrict_check.cl"};
        ...
        instance->ostream() <<"Compiling restrict_check "<<OCL_FMA<<" source -- start \n";
          ...
          if (pPath!=NULL)
            if(OCL_FMA)
              sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_restrict_check=%d ", pPath, 32,xdim0);
            else
              sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_restrict_check=%d ", pPath, 32,xdim0);
          else {
          ...
          instance->ostream() << "compiling restrict_check -- done\n";

        // Create the OpenCL kernel
        instance->opencl_instance->OPS_opencl_core.kernel[7] = clCreateKernel(instance->opencl_instance->OPS_opencl_core.program, "ops_restrict_check", &ret);
        clSafeCall( ret );

        isbuilt_restrict_check = true;
        free(source_str[0]);
      }
...
void ops_par_loop_restrict_check(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1, ops_arg arg2, ops_arg arg3) {
  ...
  buildOpenCLKernels_restrict_check(block->instance,
  xdim0);
  ...
  if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[7], 1, sizeof(cl_mem), (void*) &arg2.data_d ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[7], 2, nthread*sizeof(int), NULL));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[7], 3, sizeof(cl_int), (void*) &r_bytes2 ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[7], 4, sizeof(cl_int), (void*) arg3.data ));
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[7], 7, sizeof(cl_int), (void*) &arg_idx[1] ));
