...
//
    ...
    static bool isbuilt_mgrid_populate_kernel_3 = false;

    void buildOpenCLKernels_mgrid_populate_kernel_3(OPS_instance *instance, int xdim0) {

      //int ocl_fma = OCL_FMA;
      if(!isbuilt_mgrid_populate_kernel_3) {
        buildOpenCLKernels(instance);
        //clSafeCall( clUnloadCompiler() );
        cl_int ret;
        char* source_filename[1] = {(char*)"./OpenCL/mgrid_populate_kernel_3.cl"};
        ...
        instance->ostream() <<"Compiling mgrid_populate_kernel_3 "<<OCL_FMA<<" source -- start \n";
          ...
          if (pPath!=NULL)
            if(OCL_FMA)
              sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_mgrid_populate_kernel_3=%d ", pPath, 32,xdim0);
            else
              sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_mgrid_populate_kernel_3=%d ", pPath, 32,xdim0);
          else {
          ...
          instance->ostream() << "compiling mgrid_populate_kernel_3 -- done\n";

        // Create the OpenCL kernel
        instance->opencl_instance->OPS_opencl_core.kernel[4] = clCreateKernel(instance->opencl_instance->OPS_opencl_core.program, "ops_mgrid_populate_kernel_3", &ret);
        clSafeCall( ret );

        isbuilt_mgrid_populate_kernel_3 = true;
        free(source_str[0]);
      }
...
void ops_par_loop_mgrid_populate_kernel_3(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  buildOpenCLKernels_mgrid_populate_kernel_3(block->instance,
  xdim0);
  ...
  if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[4], 1, sizeof(cl_int), (void*) &base0 ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[4], 2, sizeof(cl_int), (void*) &arg_idx[0] ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[4], 3, sizeof(cl_int), (void*) &arg_idx[1] ));
