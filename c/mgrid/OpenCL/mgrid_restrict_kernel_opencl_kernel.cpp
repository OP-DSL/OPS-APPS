...
//
    ...
    static bool isbuilt_mgrid_restrict_kernel = false;

    void buildOpenCLKernels_mgrid_restrict_kernel(OPS_instance *instance, int xdim0, int xdim1) {

      //int ocl_fma = OCL_FMA;
      if(!isbuilt_mgrid_restrict_kernel) {
        buildOpenCLKernels(instance);
        //clSafeCall( clUnloadCompiler() );
        cl_int ret;
        char* source_filename[1] = {(char*)"./OpenCL/mgrid_restrict_kernel.cl"};
        ...
        instance->ostream() <<"Compiling mgrid_restrict_kernel "<<OCL_FMA<<" source -- start \n";
          ...
          if (pPath!=NULL)
            if(OCL_FMA)
              sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_mgrid_restrict_kernel=%d  -Dxdim1_mgrid_restrict_kernel=%d ", pPath, 32,xdim0,xdim1);
            else
              sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_mgrid_restrict_kernel=%d  -Dxdim1_mgrid_restrict_kernel=%d ", pPath, 32,xdim0,xdim1);
          else {
          ...
          instance->ostream() << "compiling mgrid_restrict_kernel -- done\n";

        // Create the OpenCL kernel
        instance->opencl_instance->OPS_opencl_core.kernel[6] = clCreateKernel(instance->opencl_instance->OPS_opencl_core.program, "ops_mgrid_restrict_kernel", &ret);
        clSafeCall( ret );

        isbuilt_mgrid_restrict_kernel = true;
        free(source_str[0]);
      }
...
void ops_par_loop_mgrid_restrict_kernel(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1, ops_arg arg2) {
  ...
  buildOpenCLKernels_mgrid_restrict_kernel(block->instance,
  xdim0,xdim1);
  ...
  if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[6], 2, sizeof(cl_int), (void*) &base0 ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[6], 3, sizeof(cl_int), (void*) &base1 ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[6], 4, sizeof(cl_int), (void*) &arg_idx[0] ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[6], 5, sizeof(cl_int), (void*) &arg_idx[1] ));
