...
//
__constant__ int dims_mgrid_populate_kernel_1 [2][1];
static int dims_mgrid_populate_kernel_1_h [2][1] = {{0}};

//user function
__device__

void mgrid_populate_kernel_1_gpu(ACC<double> &val,
  int *idx) {
...
int size1 ){
  ...
  if (size0==-1) dims_mgrid_populate_kernel_1[0][0]=0;
  ...
  arg0 += idx_x * 1*1 + idx_y * 1*1 * dims_mgrid_populate_kernel_1[0][0];

  if (idx_x < size0 && idx_y < size1) {
    ACC<double> argp0(dims_mgrid_populate_kernel_1[0][0], arg0);
    mgrid_populate_kernel_1_gpu(argp0, arg_idx);
  }
...
void ops_par_loop_mgrid_populate_kernel_1_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_mgrid_populate_kernel_1_h[0][0]) {
    dims_mgrid_populate_kernel_1_h[0][0] = xdim0;
    hipSafeCall(block->instance->ostream(), hipMemcpyToSymbol(HIP_SYMBOL(dims_mgrid_populate_kernel_1), dims_mgrid_populate_kernel_1_h, sizeof(dims_mgrid_populate_kernel_1)));
  }
  ...
  if (x_size > 0 && y_size > 0)
    hipLaunchKernelGGL(ops_mgrid_populate_kernel_1,grid ,tblock ,0 ,0 , (double *)p_a[0], arg_idx[0], arg_idx[1],x_size, y_size);
