...
int size1 ){
  ...
  if (size0==-1) dims_mgrid_populate_kernel_3[0][0]=0;
...
void ops_par_loop_mgrid_populate_kernel_3_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_mgrid_populate_kernel_3_h[0][0]) {
    ...
    hipSafeCall(block->instance->ostream(), hipMemcpyToSymbol(HIP_SYMBOL(dims_mgrid_populate_kernel_3), dims_mgrid_populate_kernel_3_h, sizeof(dims_mgrid_populate_kernel_3)));
  }
  ...
  if (x_size > 0 && y_size > 0)
    hipLaunchKernelGGL(ops_mgrid_populate_kernel_3,grid ,tblock ,0 ,0 , (double *)p_a[0], arg_idx[0], arg_idx[1],x_size, y_size);
