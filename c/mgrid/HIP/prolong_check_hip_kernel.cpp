...
int size1 ){
  ...
  if (size0==-1) dims_prolong_check[0][0]=0;
...
void ops_par_loop_prolong_check_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_prolong_check_h[0][0]) {
    ...
    hipSafeCall(block->instance->ostream(), hipMemcpyToSymbol(HIP_SYMBOL(dims_prolong_check), dims_prolong_check_h, sizeof(dims_prolong_check)));
  }
  ...
  if (x_size > 0 && y_size > 0)
    hipLaunchKernelGGL(ops_prolong_check,grid ,tblock ,nshared ,0 , (double *)p_a[0], arg_idx[0], arg_idx[1],
