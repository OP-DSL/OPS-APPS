...
int size1 ){
  ...
  if (size0==-1) dims_restrict_check[0][0]=0;
  ...
  for (int d=0; d<1; d++)
    ops_reduction_hip<OPS_MAX>(&arg2[d+(hipBlockIdx_x+ hipBlockIdx_y*hipGridDim_x)*1],arg2_l[d]);
...
void ops_par_loop_restrict_check_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_restrict_check_h[0][0]) {
    ...
    hipSafeCall(block->instance->ostream(), hipMemcpyToSymbol(HIP_SYMBOL(dims_restrict_check), dims_restrict_check_h, sizeof(dims_restrict_check)));
  }
  ...
  if (x_size > 0 && y_size > 0)
    hipLaunchKernelGGL(ops_restrict_check,grid ,tblock ,nshared ,0 , (double *)p_a[0], arg_idx[0], arg_idx[1],
