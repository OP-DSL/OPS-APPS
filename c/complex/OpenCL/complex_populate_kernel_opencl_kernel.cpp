...
//
    ...
    static bool isbuilt_complex_populate_kernel = false;

    void buildOpenCLKernels_complex_populate_kernel(OPS_instance *instance, int xdim0) {

      //int ocl_fma = OCL_FMA;
      if(!isbuilt_complex_populate_kernel) {
        buildOpenCLKernels(instance);
        //clSafeCall( clUnloadCompiler() );
        cl_int ret;
        char* source_filename[1] = {(char*)"./OpenCL/complex_populate_kernel.cl"};
        ...
        instance->ostream() <<"Compiling complex_populate_kernel "<<OCL_FMA<<" source -- start \n";
          ...
          if (pPath!=NULL)
            if(OCL_FMA)
              sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_complex_populate_kernel=%d ", pPath, 32,xdim0);
            else
              sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_complex_populate_kernel=%d ", pPath, 32,xdim0);
          else {
          ...
          instance->ostream() << "compiling complex_populate_kernel -- done\n";

        // Create the OpenCL kernel
        instance->opencl_instance->OPS_opencl_core.kernel[0] = clCreateKernel(instance->opencl_instance->OPS_opencl_core.program, "ops_complex_populate_kernel", &ret);
        clSafeCall( ret );

        isbuilt_complex_populate_kernel = true;
        free(source_str[0]);
      }
...
void ops_par_loop_complex_populate_kernel(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1, ops_arg arg2) {
  ...
  buildOpenCLKernels_complex_populate_kernel(block->instance,
  xdim0);
  ...
  int r_bytes1 = reduct_bytes/sizeof(complexd);
  ...
  if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[0], 2, nthread*sizeof(complexd), NULL));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[0], 3, sizeof(cl_int), (void*) &r_bytes1 ));
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[0], 7, sizeof(cl_int), (void*) &x_size ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[0], 8, sizeof(cl_int), (void*) &y_size ));
