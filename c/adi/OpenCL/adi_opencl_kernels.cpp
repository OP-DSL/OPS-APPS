...
void ops_decl_const_char(int dim, char const * type, int typeSize, char * dat, char const * name ) {
  ...
  if (instance->opencl_instance->OPS_opencl_core.constant == NULL) {
    instance->opencl_instance->OPS_opencl_core.constant = (cl_mem*) malloc((4)*sizeof(cl_mem));
    for ( int i=0; i<4; i++ ){
  ...
  void buildOpenCLKernels(OPS_instance *instance) {
    ...
    if(!isbuilt) {
      ...
      instance->opencl_instance->OPS_opencl_core.n_kernels = 2;
      instance->opencl_instance->OPS_opencl_core.kernel = (cl_kernel*) malloc(2*sizeof(cl_kernel));
    }
    isbuilt = true;
  }
  
//user kernel files
#include "preproc_kernel_opencl_kernel.cpp"
