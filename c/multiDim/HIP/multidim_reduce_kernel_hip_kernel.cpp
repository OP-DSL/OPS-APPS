...
int size1 ){
  ...
  if (size0==-1) dims_multidim_reduce_kernel[0][0]=0;
  ...
  for (int d=0; d<2; d++)
    ops_reduction_hip<OPS_INC>(&arg1[d+(hipBlockIdx_x+ hipBlockIdx_y*hipGridDim_x)*2],arg1_l[d]);
...
void ops_par_loop_multidim_reduce_kernel_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_multidim_reduce_kernel_h[0][0] || ydim0 != dims_multidim_reduce_kernel_h[0][1]) {
    dims_multidim_reduce_kernel_h[0][0] = xdim0;
    dims_multidim_reduce_kernel_h[0][1] = ydim0;
    hipSafeCall(block->instance->ostream(), hipMemcpyToSymbol(HIP_SYMBOL(dims_multidim_reduce_kernel), dims_multidim_reduce_kernel_h, sizeof(dims_multidim_reduce_kernel)));
  }
  ...
  if (x_size > 0 && y_size > 0)
    hipLaunchKernelGGL(ops_multidim_reduce_kernel,grid ,tblock ,nshared ,0 , (double *)p_a[0], (double *)arg1.data_d,x_size, y_size);
