...
void ops_par_loop_multidim_copy_kernel_execute(ops_kernel_descriptor *desc) {
  ...
  #ifdef OPS_DEBUG
  ops_register_args(block->instance, args, "multidim_copy_kernel");
  #endif
  ...
  double * __restrict__ src_p = (double *)(args[0].data + base0);

  int base1 = args[1].dat->base_offset;
  double * __restrict__ dest_p = (double *)(args[1].data + base1);
  ...
  for ( int n_y=start[1]; n_y<end[1]; n_y++ ){
    ...
    for ( int n_x=start[0]; n_x<end[0]; n_x++ ){
      #ifdef OPS_SOA
      const ACC<double> src(2, xdim0_multidim_copy_kernel, ydim0_multidim_copy_kernel, src_p + n_x*1 + n_y * xdim0_multidim_copy_kernel*1);
      #else
      const ACC<double> src(2, xdim0_multidim_copy_kernel, ydim0_multidim_copy_kernel, src_p + 2*(n_x*1 + n_y * xdim0_multidim_copy_kernel*1));
      #endif
      #ifdef OPS_SOA
      ACC<double> dest(2, xdim1_multidim_copy_kernel, ydim1_multidim_copy_kernel, dest_p + n_x*1 + n_y * xdim1_multidim_copy_kernel*1);
      #else
      ACC<double> dest(2, xdim1_multidim_copy_kernel, ydim1_multidim_copy_kernel, dest_p + 2*(n_x*1 + n_y * xdim1_multidim_copy_kernel*1));
      #endif
