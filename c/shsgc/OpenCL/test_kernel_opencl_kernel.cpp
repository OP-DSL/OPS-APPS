...
//
    ...
    static bool isbuilt_test_kernel = false;

    void buildOpenCLKernels_test_kernel(OPS_instance *instance, int xdim0) {
      ...
      if(!isbuilt_test_kernel) {
        ...
        instance->ostream() <<"Compiling test_kernel "<<OCL_FMA<<" source -- start \n";
          ...
          if (pPath!=NULL)
            if(OCL_FMA)
              sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_test_kernel=%d ", pPath, 32,xdim0);
            else
              sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_test_kernel=%d ", pPath, 32,xdim0);
          else {
          ...
          instance->ostream() << "compiling test_kernel -- done\n";

        // Create the OpenCL kernel
        instance->opencl_instance->OPS_opencl_core.kernel[14] = clCreateKernel(instance->opencl_instance->OPS_opencl_core.program, "ops_test_kernel", &ret);
        clSafeCall( ret );

        isbuilt_test_kernel = true;
        free(source_str[0]);
      }
...
void ops_par_loop_test_kernel(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  buildOpenCLKernels_test_kernel(block->instance,
  xdim0);
  ...
  if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[14], 2, nthread*sizeof(double), NULL));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[14], 3, sizeof(cl_int), (void*) &r_bytes1 ));
