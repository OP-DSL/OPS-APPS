...
//
    ...
    static bool isbuilt_drhoudx_kernel = false;

    void buildOpenCLKernels_drhoudx_kernel(OPS_instance *instance, int xdim0, int xdim1) {

      //int ocl_fma = OCL_FMA;
      if(!isbuilt_drhoudx_kernel) {
        ...
        instance->ostream() <<"Compiling drhoudx_kernel "<<OCL_FMA<<" source -- start \n";
          ...
          if (pPath!=NULL)
            if(OCL_FMA)
              sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_drhoudx_kernel=%d  -Dxdim1_drhoudx_kernel=%d ", pPath, 32,xdim0,xdim1);
            else
              sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_drhoudx_kernel=%d  -Dxdim1_drhoudx_kernel=%d ", pPath, 32,xdim0,xdim1);
          else {
          ...
          instance->ostream() << "compiling drhoudx_kernel -- done\n";

        // Create the OpenCL kernel
        instance->opencl_instance->OPS_opencl_core.kernel[3] = clCreateKernel(instance->opencl_instance->OPS_opencl_core.program, "ops_drhoudx_kernel", &ret);
        clSafeCall( ret );

        isbuilt_drhoudx_kernel = true;
        free(source_str[0]);
      }
...
void ops_par_loop_drhoudx_kernel(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  if (block->instance->OPS_diags > 1) {
    ops_timing_realloc(block->instance,3,"drhoudx_kernel");
  ...
  buildOpenCLKernels_drhoudx_kernel(block->instance,
  xdim0,xdim1);
  ...
  if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[3], 2, sizeof(cl_double), (void*) &dx ));
