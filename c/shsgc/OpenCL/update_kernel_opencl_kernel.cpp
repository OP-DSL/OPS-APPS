...
//
    ...
    void buildOpenCLKernels_update_kernel(OPS_instance *instance, int xdim0, int xdim1, int xdim2, int xdim3) {
      ...
      if(!isbuilt_update_kernel) {
        ...
        instance->ostream() <<"Compiling update_kernel "<<OCL_FMA<<" source -- start \n";
          ...
          if (pPath!=NULL)
            if(OCL_FMA)
              sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_update_kernel=%d  -Dxdim1_update_kernel=%d  -Dxdim2_update_kernel=%d  -Dxdim3_update_kernel=%d ", pPath, 32,xdim0,xdim1,xdim2,xdim3);
            else
              sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_update_kernel=%d  -Dxdim1_update_kernel=%d  -Dxdim2_update_kernel=%d  -Dxdim3_update_kernel=%d ", pPath, 32,xdim0,xdim1,xdim2,xdim3);
          else {
          ...
          instance->ostream() << "compiling update_kernel -- done\n";

        // Create the OpenCL kernel
        instance->opencl_instance->OPS_opencl_core.kernel[13] = clCreateKernel(instance->opencl_instance->OPS_opencl_core.program, "ops_update_kernel", &ret);
        clSafeCall( ret );
...
void ops_par_loop_update_kernel(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1, ops_arg arg2, ops_arg arg3) {
  ...
  buildOpenCLKernels_update_kernel(block->instance,
  ...
  if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[13], 6, sizeof(cl_int), (void*) &base2 ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[13], 7, sizeof(cl_int), (void*) &base3 ));
