...
//
    ...
    static bool isbuilt_drhouupdx_kernel = false;

    void buildOpenCLKernels_drhouupdx_kernel(OPS_instance *instance, int xdim0, int xdim1, int xdim2, int xdim3) {

      //int ocl_fma = OCL_FMA;
      if(!isbuilt_drhouupdx_kernel) {
        ...
        instance->ostream() <<"Compiling drhouupdx_kernel "<<OCL_FMA<<" source -- start \n";
          ...
          if (pPath!=NULL)
            if(OCL_FMA)
              sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_drhouupdx_kernel=%d  -Dxdim1_drhouupdx_kernel=%d  -Dxdim2_drhouupdx_kernel=%d  -Dxdim3_drhouupdx_kernel=%d ", pPath, 32,xdim0,xdim1,xdim2,xdim3);
            else
              sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_drhouupdx_kernel=%d  -Dxdim1_drhouupdx_kernel=%d  -Dxdim2_drhouupdx_kernel=%d  -Dxdim3_drhouupdx_kernel=%d ", pPath, 32,xdim0,xdim1,xdim2,xdim3);
          else {
          ...
          instance->ostream() << "compiling drhouupdx_kernel -- done\n";

        // Create the OpenCL kernel
        instance->opencl_instance->OPS_opencl_core.kernel[4] = clCreateKernel(instance->opencl_instance->OPS_opencl_core.program, "ops_drhouupdx_kernel", &ret);
        clSafeCall( ret );

        isbuilt_drhouupdx_kernel = true;
        free(source_str[0]);
      }
...
void ops_par_loop_drhouupdx_kernel(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1, ops_arg arg2, ops_arg arg3) {
  ...
  buildOpenCLKernels_drhouupdx_kernel(block->instance,
  ...
  if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[4], 4, sizeof(cl_double), (void*) &dx ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[4], 5, sizeof(cl_double), (void*) &gam1 ));
