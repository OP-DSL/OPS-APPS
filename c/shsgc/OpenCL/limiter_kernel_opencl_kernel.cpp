...
//
    ...
    static bool isbuilt_limiter_kernel = false;

    void buildOpenCLKernels_limiter_kernel(OPS_instance *instance, int xdim0, int xdim1, int xdim2) {

      //int ocl_fma = OCL_FMA;
      if(!isbuilt_limiter_kernel) {
        ...
        instance->ostream() <<"Compiling limiter_kernel "<<OCL_FMA<<" source -- start \n";
          ...
          if (pPath!=NULL)
            if(OCL_FMA)
              sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_limiter_kernel=%d  -Dxdim1_limiter_kernel=%d  -Dxdim2_limiter_kernel=%d ", pPath, 32,xdim0,xdim1,xdim2);
            else
              sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_limiter_kernel=%d  -Dxdim1_limiter_kernel=%d  -Dxdim2_limiter_kernel=%d ", pPath, 32,xdim0,xdim1,xdim2);
          else {
          ...
          instance->ostream() << "compiling limiter_kernel -- done\n";

        // Create the OpenCL kernel
        instance->opencl_instance->OPS_opencl_core.kernel[8] = clCreateKernel(instance->opencl_instance->OPS_opencl_core.program, "ops_limiter_kernel", &ret);
        clSafeCall( ret );

        isbuilt_limiter_kernel = true;
        free(source_str[0]);
      }
...
void ops_par_loop_limiter_kernel(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1, ops_arg arg2) {
  ...
  buildOpenCLKernels_limiter_kernel(block->instance,
  xdim0,xdim1,xdim2);
  ...
  if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[8], 3, sizeof(cl_double), (void*) &del2 ));
