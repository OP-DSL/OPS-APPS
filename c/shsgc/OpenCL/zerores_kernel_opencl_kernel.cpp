...
//
    ...
    void buildOpenCLKernels_zerores_kernel(OPS_instance *instance, int xdim0, int xdim1, int xdim2) {
      ...
      if(!isbuilt_zerores_kernel) {
        ...
        instance->ostream() <<"Compiling zerores_kernel "<<OCL_FMA<<" source -- start \n";
          ...
          if (pPath!=NULL)
            if(OCL_FMA)
              sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_zerores_kernel=%d  -Dxdim1_zerores_kernel=%d  -Dxdim2_zerores_kernel=%d ", pPath, 32,xdim0,xdim1,xdim2);
            else
              sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_zerores_kernel=%d  -Dxdim1_zerores_kernel=%d  -Dxdim2_zerores_kernel=%d ", pPath, 32,xdim0,xdim1,xdim2);
          else {
          ...
          instance->ostream() << "compiling zerores_kernel -- done\n";

        // Create the OpenCL kernel
        instance->opencl_instance->OPS_opencl_core.kernel[2] = clCreateKernel(instance->opencl_instance->OPS_opencl_core.program, "ops_zerores_kernel", &ret);
        clSafeCall( ret );
...
void ops_par_loop_zerores_kernel(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1, ops_arg arg2) {
  ...
  buildOpenCLKernels_zerores_kernel(block->instance,
  xdim0,xdim1,xdim2);
