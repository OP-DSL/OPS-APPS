...
//
    ...
    static bool isbuilt_initialize_kernel = false;

    void buildOpenCLKernels_initialize_kernel(OPS_instance *instance, int xdim0, int xdim1, int xdim2, int xdim3, int xdim4) {

      //int ocl_fma = OCL_FMA;
      if(!isbuilt_initialize_kernel) {
        ...
        instance->ostream() <<"Compiling initialize_kernel "<<OCL_FMA<<" source -- start \n";
          ...
          if (pPath!=NULL)
            if(OCL_FMA)
              sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_initialize_kernel=%d  -Dxdim1_initialize_kernel=%d  -Dxdim2_initialize_kernel=%d  -Dxdim3_initialize_kernel=%d  -Dxdim4_initialize_kernel=%d ", pPath, 32,xdim0,xdim1,xdim2,xdim3,xdim4);
            else
              sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_initialize_kernel=%d  -Dxdim1_initialize_kernel=%d  -Dxdim2_initialize_kernel=%d  -Dxdim3_initialize_kernel=%d  -Dxdim4_initialize_kernel=%d ", pPath, 32,xdim0,xdim1,xdim2,xdim3,xdim4);
          else {
          ...
          instance->ostream() << "compiling initialize_kernel -- done\n";

        // Create the OpenCL kernel
        instance->opencl_instance->OPS_opencl_core.kernel[0] = clCreateKernel(instance->opencl_instance->OPS_opencl_core.program, "ops_initialize_kernel", &ret);
        clSafeCall( ret );
...
void ops_par_loop_initialize_kernel(char const *name, ops_block block, int dim, int* range,
 ...
 ops_arg arg4, ops_arg arg5) {
  ...
  buildOpenCLKernels_initialize_kernel(block->instance,
  ...
  if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[0], 5, sizeof(cl_double), (void*) &xmin ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[0], 6, sizeof(cl_double), (void*) &dx ));
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[0], 14, sizeof(cl_double), (void*) &lambda ));
