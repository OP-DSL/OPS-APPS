...
int size0 ){
  ...
  if (size0==-1) dims_fact_kernel[0][0]=0;
...
void ops_par_loop_fact_kernel_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_fact_kernel_h[0][0] || xdim1 != dims_fact_kernel_h[1][0]) {
    ...
    hipSafeCall(block->instance->ostream(), hipMemcpyToSymbol(HIP_SYMBOL(dims_fact_kernel), dims_fact_kernel_h, sizeof(dims_fact_kernel)));
  }
  ...
  if (x_size > 0)
    hipLaunchKernelGGL(ops_fact_kernel,grid ,tblock ,0 ,0 , (double *)p_a[0], (double *)p_a[1],x_size);
