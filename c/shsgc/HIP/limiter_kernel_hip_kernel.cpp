...
int size0 ){
  ...
  if (size0==-1) dims_limiter_kernel[0][0]=0;
...
void ops_par_loop_limiter_kernel_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_limiter_kernel_h[0][0] || xdim1 != dims_limiter_kernel_h[1][0] || xdim2 != dims_limiter_kernel_h[2][0]) {
    ...
    hipSafeCall(block->instance->ostream(), hipMemcpyToSymbol(HIP_SYMBOL(dims_limiter_kernel), dims_limiter_kernel_h, sizeof(dims_limiter_kernel)));
  }
  ...
  if (x_size > 0)
    hipLaunchKernelGGL(ops_limiter_kernel,grid ,tblock ,0 ,0 , (double *)p_a[0], (double *)p_a[1],
         (double *)p_a[2],x_size);
