...
int size0 ){
  ...
  if (size0==-1) dims_test_kernel[0][0]=0;
...
void ops_par_loop_test_kernel_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_test_kernel_h[0][0]) {
    ...
    hipSafeCall(block->instance->ostream(), hipMemcpyToSymbol(HIP_SYMBOL(dims_test_kernel), dims_test_kernel_h, sizeof(dims_test_kernel)));
  }
  ...
  if (x_size > 0)
    hipLaunchKernelGGL(ops_test_kernel,grid ,tblock ,nshared ,0 , (double *)p_a[0], (double *)arg1.data_d,x_size);
