...
int size0 ){
  ...
  if (size0==-1) dims_drhoudx_kernel[0][0]=0;
...
void ops_par_loop_drhoudx_kernel_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_drhoudx_kernel_h[0][0] || xdim1 != dims_drhoudx_kernel_h[1][0]) {
    ...
    hipSafeCall(block->instance->ostream(), hipMemcpyToSymbol(HIP_SYMBOL(dims_drhoudx_kernel), dims_drhoudx_kernel_h, sizeof(dims_drhoudx_kernel)));
  }
  ...
  if (x_size > 0)
    hipLaunchKernelGGL(ops_drhoudx_kernel,grid ,tblock ,0 ,0 , (double *)p_a[0], (double *)p_a[1],x_size);
