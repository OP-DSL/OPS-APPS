...
//
    ...
    static bool isbuilt_mblock_populate_kernel = false;

    void buildOpenCLKernels_mblock_populate_kernel(OPS_instance *instance, int xdim0) {
      ...
      if(!isbuilt_mblock_populate_kernel) {
        ...
        instance->ostream() <<"Compiling mblock_populate_kernel "<<OCL_FMA<<" source -- start \n";
          ...
          if (pPath!=NULL)
            if(OCL_FMA)
              sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_mblock_populate_kernel=%d ", pPath, 32,xdim0);
            else
              sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_mblock_populate_kernel=%d ", pPath, 32,xdim0);
          else {
          ...
          instance->ostream() << "compiling mblock_populate_kernel -- done\n";

        // Create the OpenCL kernel
        instance->opencl_instance->OPS_opencl_core.kernel[0] = clCreateKernel(instance->opencl_instance->OPS_opencl_core.program, "ops_mblock_populate_kernel", &ret);
        clSafeCall( ret );

        isbuilt_mblock_populate_kernel = true;
        free(source_str[0]);
      }
...
void ops_par_loop_mblock_populate_kernel(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  buildOpenCLKernels_mblock_populate_kernel(block->instance,
  xdim0);
