...
//
    ...
    static bool isbuilt_access_populate_kernel = false;

    void buildOpenCLKernels_access_populate_kernel(OPS_instance *instance, int xdim0) {

      //int ocl_fma = OCL_FMA;
      if(!isbuilt_access_populate_kernel) {
        buildOpenCLKernels(instance);
        //clSafeCall( clUnloadCompiler() );
        cl_int ret;
        char* source_filename[1] = {(char*)"./OpenCL/access_populate_kernel.cl"};
        ...
        instance->ostream() <<"Compiling access_populate_kernel "<<OCL_FMA<<" source -- start \n";
          ...
          if (pPath!=NULL)
            if(OCL_FMA)
              sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_access_populate_kernel=%d ", pPath, 32,xdim0);
            else
              sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_access_populate_kernel=%d ", pPath, 32,xdim0);
          else {
          ...
          instance->ostream() << "compiling access_populate_kernel -- done\n";

        // Create the OpenCL kernel
        instance->opencl_instance->OPS_opencl_core.kernel[0] = clCreateKernel(instance->opencl_instance->OPS_opencl_core.program, "ops_access_populate_kernel", &ret);
        clSafeCall( ret );

        isbuilt_access_populate_kernel = true;
        free(source_str[0]);
      }
...
void ops_par_loop_access_populate_kernel(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  buildOpenCLKernels_access_populate_kernel(block->instance,
  xdim0);
