...
//
    ...
    static bool isbuilt_set_field_kernel = false;

    void buildOpenCLKernels_set_field_kernel(OPS_instance *instance, int xdim0, int xdim1) {

      //int ocl_fma = OCL_FMA;
      if(!isbuilt_set_field_kernel) {
        buildOpenCLKernels(instance);
        //clSafeCall( clUnloadCompiler() );
        cl_int ret;
        char* source_filename[1] = {(char*)"./OpenCL/set_field_kernel.cl"};
        ...
        instance->ostream() <<"Compiling set_field_kernel "<<OCL_FMA<<" source -- start \n";
          ...
          if (pPath!=NULL)
            if(OCL_FMA)
              sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_set_field_kernel=%d  -Dxdim1_set_field_kernel=%d ", pPath, 32,xdim0,xdim1);
            else
              sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_set_field_kernel=%d  -Dxdim1_set_field_kernel=%d ", pPath, 32,xdim0,xdim1);
          else {
          ...
          instance->ostream() << "compiling set_field_kernel -- done\n";

        // Create the OpenCL kernel
        instance->opencl_instance->OPS_opencl_core.kernel[15] = clCreateKernel(instance->opencl_instance->OPS_opencl_core.program, "ops_set_field_kernel", &ret);
        clSafeCall( ret );

        isbuilt_set_field_kernel = true;
        free(source_str[0]);
      }
...
void ops_par_loop_set_field_kernel(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  buildOpenCLKernels_set_field_kernel(block->instance,
  xdim0,xdim1);
  ...
  if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[15], 2, sizeof(cl_int), (void*) &base0 ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[15], 3, sizeof(cl_int), (void*) &base1 ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[15], 4, sizeof(cl_int), (void*) &x_size ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[15], 5, sizeof(cl_int), (void*) &y_size ));
