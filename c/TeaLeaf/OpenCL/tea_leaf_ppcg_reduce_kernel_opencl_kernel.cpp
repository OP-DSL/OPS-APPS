...
//
    ...
    static bool isbuilt_tea_leaf_ppcg_reduce_kernel = false;

    void buildOpenCLKernels_tea_leaf_ppcg_reduce_kernel(OPS_instance *instance, int xdim0, int xdim1, int xdim2) {

      //int ocl_fma = OCL_FMA;
      if(!isbuilt_tea_leaf_ppcg_reduce_kernel) {
        buildOpenCLKernels(instance);
        //clSafeCall( clUnloadCompiler() );
        cl_int ret;
        char* source_filename[1] = {(char*)"./OpenCL/tea_leaf_ppcg_reduce_kernel.cl"};
        ...
        instance->ostream() <<"Compiling tea_leaf_ppcg_reduce_kernel "<<OCL_FMA<<" source -- start \n";
          ...
          if (pPath!=NULL)
            if(OCL_FMA)
              sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_tea_leaf_ppcg_reduce_kernel=%d  -Dxdim1_tea_leaf_ppcg_reduce_kernel=%d  -Dxdim2_tea_leaf_ppcg_reduce_kernel=%d ", pPath, 32,xdim0,xdim1,xdim2);
            else
              sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_tea_leaf_ppcg_reduce_kernel=%d  -Dxdim1_tea_leaf_ppcg_reduce_kernel=%d  -Dxdim2_tea_leaf_ppcg_reduce_kernel=%d ", pPath, 32,xdim0,xdim1,xdim2);
          else {
          ...
          instance->ostream() << "compiling tea_leaf_ppcg_reduce_kernel -- done\n";

        // Create the OpenCL kernel
        instance->opencl_instance->OPS_opencl_core.kernel[48] = clCreateKernel(instance->opencl_instance->OPS_opencl_core.program, "ops_tea_leaf_ppcg_reduce_kernel", &ret);
        clSafeCall( ret );

        isbuilt_tea_leaf_ppcg_reduce_kernel = true;
        free(source_str[0]);
      }
...
void ops_par_loop_tea_leaf_ppcg_reduce_kernel(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1, ops_arg arg2, ops_arg arg3) {
  ...
  buildOpenCLKernels_tea_leaf_ppcg_reduce_kernel(block->instance,
  xdim0,xdim1,xdim2);
  ...
  if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[48], 3, sizeof(cl_mem), (void*) &arg3.data_d ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[48], 4, nthread*sizeof(double), NULL));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[48], 5, sizeof(cl_int), (void*) &r_bytes3 ));
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[48], 8, sizeof(cl_int), (void*) &base2 ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[48], 9, sizeof(cl_int), (void*) &x_size ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[48], 10, sizeof(cl_int), (void*) &y_size ));
