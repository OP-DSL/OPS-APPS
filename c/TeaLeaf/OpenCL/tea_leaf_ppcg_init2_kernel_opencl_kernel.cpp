...
//
    ...
    static bool isbuilt_tea_leaf_ppcg_init2_kernel = false;

    void buildOpenCLKernels_tea_leaf_ppcg_init2_kernel(OPS_instance *instance, int xdim0, int xdim1, int xdim2, int xdim3) {

      //int ocl_fma = OCL_FMA;
      if(!isbuilt_tea_leaf_ppcg_init2_kernel) {
        buildOpenCLKernels(instance);
        //clSafeCall( clUnloadCompiler() );
        cl_int ret;
        char* source_filename[1] = {(char*)"./OpenCL/tea_leaf_ppcg_init2_kernel.cl"};
        ...
        instance->ostream() <<"Compiling tea_leaf_ppcg_init2_kernel "<<OCL_FMA<<" source -- start \n";
          ...
          if (pPath!=NULL)
            if(OCL_FMA)
              sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_tea_leaf_ppcg_init2_kernel=%d  -Dxdim1_tea_leaf_ppcg_init2_kernel=%d  -Dxdim2_tea_leaf_ppcg_init2_kernel=%d  -Dxdim3_tea_leaf_ppcg_init2_kernel=%d ", pPath, 32,xdim0,xdim1,xdim2,xdim3);
            else
              sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_tea_leaf_ppcg_init2_kernel=%d  -Dxdim1_tea_leaf_ppcg_init2_kernel=%d  -Dxdim2_tea_leaf_ppcg_init2_kernel=%d  -Dxdim3_tea_leaf_ppcg_init2_kernel=%d ", pPath, 32,xdim0,xdim1,xdim2,xdim3);
          else {
          ...
          instance->ostream() << "compiling tea_leaf_ppcg_init2_kernel -- done\n";

        // Create the OpenCL kernel
        instance->opencl_instance->OPS_opencl_core.kernel[44] = clCreateKernel(instance->opencl_instance->OPS_opencl_core.program, "ops_tea_leaf_ppcg_init2_kernel", &ret);
        clSafeCall( ret );

        isbuilt_tea_leaf_ppcg_init2_kernel = true;
        free(source_str[0]);
      }
...
void ops_par_loop_tea_leaf_ppcg_init2_kernel(char const *name, ops_block block, int dim, int* range,
 ...
 ops_arg arg4) {
  ...
  buildOpenCLKernels_tea_leaf_ppcg_init2_kernel(block->instance,
  ...
  if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[44], 4, sizeof(cl_double), (void*) arg4.data ));
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[44], 7, sizeof(cl_int), (void*) &base2 ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[44], 8, sizeof(cl_int), (void*) &base3 ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[44], 9, sizeof(cl_int), (void*) &x_size ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[44], 10, sizeof(cl_int), (void*) &y_size ));
