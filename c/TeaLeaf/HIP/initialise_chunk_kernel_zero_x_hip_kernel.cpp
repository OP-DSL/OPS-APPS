...
int size1 ){
  ...
  if (size0==-1) dims_initialise_chunk_kernel_zero_x[0][0]=0;
...
void ops_par_loop_initialise_chunk_kernel_zero_x_execute(ops_kernel_descriptor *desc) {
  ...
  if (block->instance->OPS_diags > 1) {
    ops_timing_realloc(block->instance,6,"initialise_chunk_kernel_zero_x");
  ...
  if (xdim0 != dims_initialise_chunk_kernel_zero_x_h[0][0]) {
    ...
    hipSafeCall(block->instance->ostream(), hipMemcpyToSymbol(HIP_SYMBOL(dims_initialise_chunk_kernel_zero_x), dims_initialise_chunk_kernel_zero_x_h, sizeof(dims_initialise_chunk_kernel_zero_x)));
  }
  ...
  if (x_size > 0 && y_size > 0)
    hipLaunchKernelGGL(ops_initialise_chunk_kernel_zero_x,grid ,tblock ,0 ,0 , (double *)p_a[0],x_size, y_size);
