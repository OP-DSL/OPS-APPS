...
int size1 ){
  ...
  if (size0==-1) dims_tea_leaf_init_zero_kernel[0][0]=0;
...
void ops_par_loop_tea_leaf_init_zero_kernel_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_tea_leaf_init_zero_kernel_h[0][0]) {
    ...
    hipSafeCall(block->instance->ostream(), hipMemcpyToSymbol(HIP_SYMBOL(dims_tea_leaf_init_zero_kernel), dims_tea_leaf_init_zero_kernel_h, sizeof(dims_tea_leaf_init_zero_kernel)));
  }
  ...
  if (x_size > 0 && y_size > 0)
    hipLaunchKernelGGL(ops_tea_leaf_init_zero_kernel,grid ,tblock ,0 ,0 , (double *)p_a[0],x_size, y_size);
