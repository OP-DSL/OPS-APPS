...
int size1 ){
  ...
  if (size0==-1) dims_tea_leaf_init_zero2_kernel[0][0]=0;


  int idx_y = hipBlockDim_y * hipBlockIdx_y + hipThreadIdx_y;
  int idx_x = hipBlockDim_x * hipBlockIdx_x + hipThreadIdx_x;

  arg0 += idx_x * 1*1 + idx_y * 1*1 * dims_tea_leaf_init_zero2_kernel[0][0];
...
#else
void ops_par_loop_tea_leaf_init_zero2_kernel_execute(ops_kernel_descriptor *desc) {
  int dim = desc->dim;
  #if OPS_MPI
  ...
  if (xdim0 != dims_tea_leaf_init_zero2_kernel_h[0][0] || xdim1 != dims_tea_leaf_init_zero2_kernel_h[1][0]) {
    dims_tea_leaf_init_zero2_kernel_h[0][0] = xdim0;
    dims_tea_leaf_init_zero2_kernel_h[1][0] = xdim1;
    hipSafeCall(block->instance->ostream(), hipMemcpyToSymbol(HIP_SYMBOL(dims_tea_leaf_init_zero2_kernel), dims_tea_leaf_init_zero2_kernel_h, sizeof(dims_tea_leaf_init_zero2_kernel)));
  }
  ...
  if (x_size > 0 && y_size > 0)
    hipLaunchKernelGGL(ops_tea_leaf_init_zero2_kernel,grid ,tblock ,0 ,0 , (double *)p_a[0], (double *)p_a[1],x_size, y_size);
