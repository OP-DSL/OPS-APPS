...
int size1 ){
  ...
  if (size0==-1) dims_initialise_chunk_kernel_zero_y[0][0]=0;
...
void ops_par_loop_initialise_chunk_kernel_zero_y_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_initialise_chunk_kernel_zero_y_h[0][0]) {
    ...
    hipSafeCall(block->instance->ostream(), hipMemcpyToSymbol(HIP_SYMBOL(dims_initialise_chunk_kernel_zero_y), dims_initialise_chunk_kernel_zero_y_h, sizeof(dims_initialise_chunk_kernel_zero_y)));
  }
  ...
  if (x_size > 0 && y_size > 0)
    hipLaunchKernelGGL(ops_initialise_chunk_kernel_zero_y,grid ,tblock ,0 ,0 , (double *)p_a[0],x_size, y_size);
