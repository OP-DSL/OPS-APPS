...
void ops_par_loop_tea_leaf_yeqx_kernel_execute(ops_kernel_descriptor *desc) {
  ...
  #ifdef OPS_DEBUG
  ops_register_args(block->instance, args, "tea_leaf_yeqx_kernel");
  #endif
  ...
  int xdim0_tea_leaf_yeqx_kernel = args[0].dat->size[0];
  int xdim1_tea_leaf_yeqx_kernel = args[1].dat->size[0];
  ...
  for ( int n_y=start[1]; n_y<end[1]; n_y++ ){
    ...
    for ( int n_x=start[0]; n_x<end[0]; n_x++ ){
      ACC<double> p(xdim0_tea_leaf_yeqx_kernel, p_p + n_x*1 + n_y * xdim0_tea_leaf_yeqx_kernel*1);
      const ACC<double> x(xdim1_tea_leaf_yeqx_kernel, x_p + n_x*1 + n_y * xdim1_tea_leaf_yeqx_kernel*1);
