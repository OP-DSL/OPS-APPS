...
void ops_par_loop_set_field_kernel_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_set_field_kernel_h[0][0] || xdim1 != dims_set_field_kernel_h[1][0]) {
    ...
    cutilSafeCall(block->instance->ostream(), cudaMemcpyToSymbol( dims_set_field_kernel, dims_set_field_kernel_h, sizeof(dims_set_field_kernel)));
  }
  ...
  if (x_size > 0 && y_size > 0)
    ops_set_field_kernel<<<grid, tblock >>> (  (double *)p_a[0], (double *)p_a[1],x_size, y_size);
